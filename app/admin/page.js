...
export default async function AdminPage() {
  ...
  // САЙЖРУУЛАЛТ: Таны сайтын бодит хаягийг шууд зааж өгөв
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 p-4">
      <div className="flex justify-between items-center bg-slate-900 text-white p-6 rounded-2xl shadow-xl">
        <div>
          <h1 className="text-2xl font-black tracking-tight text-white">Админ Хянах Самбар</h1>
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href="/" className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg text-sm font-bold transition-all no-underline">
            Гарах
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:border-blue-500 transition-all group">
          ...
          <p className="text-slate-500 text-xs mt-1">Нийт <span className="font-bold text-blue-600">{productCount}</span> бараа бүртгэлтэй</p>
          <Link href="/products" className="mt-4 inline-block text-blue-600 text-sm font-bold hover:underline no-underline">Удирдах →</Link>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm"><h3 className="font-bold text-slate-800">Статистик</h3><p className="text-slate-500 text-xs mt-1">Одоогоор өгөгдөл байхгүй</p></div>
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm"><h3 className="font-bold text-slate-800">Хандах эрх</h3><p className="text-slate-500 text-xs mt-1">Зөвхөн супер админ</p></div>
      </div>
    </div>
  );
}
