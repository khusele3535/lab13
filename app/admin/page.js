...
export default async function AdminPage() {
  ...
  // Орчноо мэдэрч URL-аа тохируулах (Vercel эсвэл Localhost)
  ...
  try {
    // API-аас өгөгдөл татах (Бүтэн URL ашиглана)
  ...
  // Хэрэв өгөгдөл татахад алдаа гарвал харуулах UI
  if (fetchError) {
    return (
      <div className="p-10 text-center bg-red-50 text-red-600 rounded-2xl m-6 border border-red-100">
        ...
        <p className="text-sm mt-2 font-medium">API сервертэй холбогдож чадсангүй.</p>
        <p className="text-xs mt-4 text-red-400">Тохиргоо: {baseUrl}</p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:border-blue-500 transition-all group">
          ...
          <p className="text-slate-500 text-xs mt-1">
            Нийт <span className="font-bold text-blue-600">{productCount}</span> бараа бүртгэлтэй
          </p>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <p className="text-slate-500 text-xs mt-1">Одоогоор өгөгдөл байхгүй</p>
        </div>
